...
import { TransactionModal, WithdrawSuccessDetails } from "@/components/transaction-modal"
...
export default function VaultWithdraw({ onSuccess }: VaultWithdrawProps) {
  ...
  const vaultName = isAave ? "Aave v3" : "Morpho"
  ...
  // Withdraw hook
  const { withdraw, isPending, isConfirming, isSuccess, hash } = useWithdrawFromVault(isAave)
  ...
  const handleWithdraw = async (e: React.FormEvent) => {
    ...
    try {
      ...
      modal.setError(err.message || "Failed to withdraw")
    }
  }
  ...
  const sharesBurned = (withdrawAmount * 1).toFixed(6) // Simplified calculation
  ...
  return (
    <>
      <TransactionModal
        ...
        confirmMessage="Processing your withdrawal request..."
        pendingMessage="Your withdrawal is being processed on the blockchain..."
        successMessage={`Successfully withdrew from ${vaultName}!`}
        successDetails={
          amount && (
